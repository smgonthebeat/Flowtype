...
4. The first click of **Prepare Flowtype**, which confirms the approximately 1.9 GB default Qwen3-ASR 0.6B model download.

The model download is separate from the repository checkout and may take time. Flowtype displays progress while downloading and preparing the model. Model files are stored under Flowtype's Application Support directory, not in the repository.
