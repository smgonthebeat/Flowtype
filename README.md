...
> **Release status:** [`v0.1.0-preview.3`](https://github.com/smgonthebeat/Flowtype/releases/tag/v0.1.0-preview.3) Apple Silicon DMG 已开放下载。它使用 local development signature，**没有 Apple Developer ID，也未经过 Apple notarization**；macOS 会拦截首次启动，需要用户在 System Settings → Privacy & Security 中明确选择 Open Anyway。
...
[校验 SHA-256](https://github.com/smgonthebeat/Flowtype/releases/download/v0.1.0-preview.3/Flowtype.dmg.sha256) · [查看 Release](https://github.com/smgonthebeat/Flowtype/releases/tag/v0.1.0-preview.3) · [GitHub Repository](https://github.com/smgonthebeat/Flowtype)
...
首次点击「一键准备 Flowtype」即确认本次下载：Flowtype 会请求 Microphone 与 Accessibility permissions，自动从 Hugging Face 下载约 1.9 GB 的默认 Qwen3-ASR 0.6B model，并在界面显示 download / preparation progress。Accessibility 仍需在 System Settings 中由用户确认；返回 Flowtype 后流程会自动继续。模型权重不在本仓库中，也不包含在源码 archive 中。

## 开始使用

普通用户不需要安装 Swift 或自己编译。请只从官方 [`v0.1.0-preview.3` Release](https://github.com/smgonthebeat/Flowtype/releases/tag/v0.1.0-preview.3) 下载 `Flowtype.dmg`，打开后按照窗口中的箭头把 `Flowtype.app` 拖进 Applications。
